...
import numpy as np
import cv2
from incomplete_region import getCoordinate
...
def final(path):
    result, classify_point, point_line, domain_line = getCoordinate(path)
    match_point, dict, point_line, side_match_point, edge_point_line = point_match(classify_point, point_line,
                                                                                   domain_line, result)

    return dict, point_line, result, side_match_point, edge_point_line, classify_point, domain_line

if __name__ == '__main__':
    path = '/home/tuyanglin/fingerprint/reconstruction/train_set/Left_loop/Left_10_O_v1_small_noise3_thinned.png'
    final(path)
